...
export const useUserStore = create<UserState>((set, get) => ({
  ...
  setMetrics: (weight, height, age) => {
    ...
    try {
      const saved = localStorage.getItem('nutriguia_body_metrics')
      const data = { weight, height, age, gender: get().gender }
      localStorage.setItem('nutriguia_body_metrics', JSON.stringify(data))
    } catch {}
  },
  ...
  getCalorieNeeds: () => {
    return Math.round(get().getBmr() * 1.55)
  },
  ...
  getPlanLabel: () => {
    ...
    switch (plan) {
      case 'PRO':
        return { name: 'Pro', color: '#8b5cf6', badge: '⭐ PRO' }
      case 'ASESORADO':
        return { name: 'Asesorado', color: '#f59e0b', badge: '👑 ASESORADO' }
      default:
        return { name: 'Gratis', color: '#6b7280', badge: 'LIBRE' }
    }
  },
}))
